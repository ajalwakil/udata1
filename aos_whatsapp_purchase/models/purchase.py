...
class PurchaseOrder(models.Model):
    ...
    def get_link(self):
        for purchase in self:
            base_url = purchase.get_base_url()
            share_url = purchase._get_share_url(redirect=True, signup_partner=True)
    ...
    def send_whatsapp_automatic(self):
        for purchase in self:
            ...
            template_id = self.env.ref('aos_whatsapp_purchase.purchase_validate_status', raise_if_not_found=False)
            if self._get_whatsapp_server() and self._get_whatsapp_server().status == 'authenticated':
                ...
                # }
                #===============================================================             
                template = template_id.generate_email(purchase.id, ['body_html'])
                body = template.get('body_html')
                subject = template.get('subject')
                try:
                    body = body.replace('_PARTNER_', purchase.partner_id.name)
                except:
                    _logger.warning('Failed to send Message to WhatsApp number %s', purchase.partner_id.whatsapp)
                attachment_ids = []
                chatIDs = []
                message_data = {}
                send_message = {}
                ...
                if purchase.partner_id:
                    partners = purchase.partner_id
                    # if purchase.partner_id.child_ids:
                    #     #ADDED CHILD FROM PARTNER
                    #     for partner in purchase.partner_id.child_ids:
                    #         partners += partner                      
                for partner in partners:
                    if partner.country_id and partner.whatsapp:
                        ...
                        message_data = {
                            'method': 'sendMessage',
                            'phone': whatsapp,
                            'body': html2text.html2text(body) + purchase.get_link(),
                            'origin': purchase.name,
                            'link': purchase.get_link(),
                        }
                        if partner.chat_id:
                            message_data.update({'chatId': partner.chat_id, 'phone': '', 'origin': purchase.name, 'link': purchase.get_link()})
                ...
                vals = WhatsappComposeMessage._prepare_mail_message(self.env.user.partner_id.id, AllchatIDs, purchase and purchase.id,  'purchase.order', body, message_data, subject, partners.ids, attachment_ids, send_message, status)
