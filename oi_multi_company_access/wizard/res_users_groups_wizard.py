...
class UserCompanyAccessWizard(models.TransientModel):
    ...
    def _group_domain(self):
        category_ids = self.env.ref("base.module_category_hidden") + self.env.ref("base.module_category_user_type")
    ...
    company_ids = fields.Many2many(related='user_id.company_ids')
    ...
    def process(self):
        if not self.user_id.company_group_ids and len(self.user_id.company_ids) > 1:
            group_ids = self.env['res.groups'].search(self._group_domain() + [('id', 'in', self.user_id.groups_id.ids)])
            for group in group_ids:
                self.env['res.users.groups'].create({
                    'user_id': self.user_id.id,
                    'group_id': group.id,
                    'company_ids' : [(6,0,self.user_id.company_ids.ids)]
                    })
