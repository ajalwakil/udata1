...
class AccountPayment(models.Model):
    ...
    @api.depends('vz_bank_charge')
    def _compute_vat_bank_charges(self):
        self.vat_bank_charges = False
        self.total_vat_bank_charges = False
        for rec in self:
            if rec.vz_bank_charge:
                rec.vat_bank_charges = (rec.vz_bank_charge * 15) / 100
                rec.total_vat_bank_charges = rec.vz_bank_charge + rec.vat_bank_charges
            ...
            if self.env.company.enable_bank_charges:
                ...
                for pay in self.with_context(skip_account_move_synchronization=True):
                    ...
                    if 'line_ids' in changed_fields:
                        ...
                        if writeoff_lines and len(
                                writeoff_lines.account_id) != 1 and not self.env.company.enable_bank_charges:
                            raise UserError(_(
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        if self.vz_bank_charge:
            ...
            if self.env.company.enable_bank_charges and self.vz_bank_charge:
                charge_list = []
        ...
        if self.vat_bank_charges:
            default_line_name = self.env['account.move.line']._get_default_line_name(
                _("Internal Transfer") if self.is_internal_transfer else payment_display_name[
                    '%s-%s' % (self.payment_type, self.partner_type)],
                self.vat_bank_charges,
                self.currency_id,
                self.date,
                partner=self.partner_id,
            )
            ...
            debit = {
                ...
                'amount_currency': liquidity_amount_currency,
                'currency_id': currency_id,
                'debit': self.vat_bank_charges,
                'credit': 0,
                'partner_id': self.partner_id.id,
                # 'account_id':  self.outstanding_account_id.id,
                'account_id': self.journal_id.vat_bank_charge_account.id,
            },
            credit = {
                'name': default_line_name,
                'date_maturity': self.date,
                ...
                'currency_id': currency_id,
                'debit': 0,
                'credit': self.vat_bank_charges,
                'partner_id': self.partner_id.id,
                # 'account_id': self.destination_account_id.id,
                'account_id': self.journal_id.default_account_id.id,

            },
            if self.env.company.enable_bank_charges and self.vat_bank_charges:
                charge_list = []
                charge_list.extend(debit)
                charge_list.extend(credit)
                charge_list.extend(res)
